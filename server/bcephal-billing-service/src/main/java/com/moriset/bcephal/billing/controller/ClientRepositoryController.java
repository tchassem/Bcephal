...
import com.moriset.bcephal.billing.service.ClientRepositoryService;
 ...
 */
@RestController
@RequestMapping("/billing/client-repository")
@Slf4j
public class ClientRepositoryController extends BaseController<Grille, BrowserData> {
	...
	@Override
	protected ClientRepositoryService getService() {
	...
	public ResponseEntity<?> getGridColumns(JwtAuthenticationToken principal, @PathVariable("gridId") Long gridId,
		...
		try {
			List<GrilleColumn> columns = clientRepositoryService.getColumns(gridId);
	...
	public ResponseEntity<?> publish(@PathVariable("gridId") Long gridId, 
		...
		log.debug("Call of publish client repository");
		try {
			...
			log.error("Unexpected error while publishing client repository.", e);
	...
	public ResponseEntity<?> resetPublication(@PathVariable("gridId") Long gridId, 
		...
		log.debug("Call of Reset client repository publication");
		try {
			...
			log.error("Unexpected error while resetting client repository publication.", e);
