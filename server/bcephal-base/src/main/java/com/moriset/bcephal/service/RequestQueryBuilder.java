...
public class RequestQueryBuilder<B> {
	...
	@SuppressWarnings("rawtypes")
	private Predicate buildEnumCriteria(String joinAttributeName, List<Enum> startWithEnumValues, String name) {
		if (startWithEnumValues != null && startWithEnumValues.size() > 0) {
			List<Predicate> predicat_ = new ArrayList<>();
			startWithEnumValues.forEach(value -> {
				predicat_.add(Equals(joinAttributeName, value, name));
			});
			Predicate[] predicat = predicat_.toArray(new Predicate[predicat_.size()]);
			return criteriaBuilder.or(predicat);
		}else {
	...
	private Predicate endsWithCriteria(String joinAttributeName, String value, String name) {
		return likeCriteria(joinAttributeName, "%".concat(value), name);
	}

	private Predicate startWithCriteria(String joinAttributeName, String value, String name) {
		return likeCriteria(joinAttributeName, value + "%", name);
	}
	...
	private Predicate Equals(String joinAttributeName, Date value, String filedName) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(value);
		calendar.set(Calendar.HOUR, 0);
		calendar.set(Calendar.MINUTE, 0);
		...
		calendar.set(Calendar.MILLISECOND, 59);
		Date date2 = calendar.getTime();
		return criteriaBuilder.between((Expression) root.join(joinAttributeName).get(filedName).as(Date.class).alias(joinAttributeName + "_" + filedName), date1, date2);
	}
//	
