...
import com.moriset.bcephal.settings.config.HashMapHttpMessageConverter;
...
@Slf4j
public class IncrementalNumberControllerIntegrationTests {
	@LocalServerPort
	private int port;
	...
	public void setUp() throws Exception {
		baseUrl = "http://localhost:".concat("" + port).concat("/settings/label");
		buildToken();
	}
	
	@Test
	@DisplayName("Save Incremental number")
	@Order(1)
	@Commit
	public void saveIncrmentalNbrTest() throws Exception {
		
		IncrementalNumber incrementalNumber = IncrementalNumber.builder()
				.creationDate(new Timestamp(System.currentTimeMillis()))
				.modificationDate(new Timestamp(System.currentTimeMillis()))
				.incrementValue(1)
				.currentValue(1L).initialValue(0L).name("valueTest")
				.build() ;
		Assertions.assertThat(incrementalNumber).isNotNull();
		//Assertions.assertThat(measure.getId()).isNull();
		...
		ResponseEntity<?> response = restClient.post().uri(baseUrl + "/save")
		    ...
		    .body(incrementalNumber).retrieve().toEntity(IncrementalNumber.class);
		
		Assertions.assertThat(response).isNotNull();
		Assertions.assertThat(response.getStatusCode()).isNotNull();
		Assertions.assertThat(response.getStatusCode().value()).isEqualTo(200);
		//ListChangeHandler<Model> newModel = response.getBody();
		//Assertions.assertThat(newModel).isNotNull();
		//Assertions.assertThat(newMeasure.getChildrenListChangeHandler().getItems().size() == measure.getChildrenListChangeHandler().getItems().size()).isTrue();
	}
