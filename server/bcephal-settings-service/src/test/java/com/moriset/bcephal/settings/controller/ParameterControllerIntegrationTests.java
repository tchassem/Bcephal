...
@Slf4j
public class ParameterControllerIntegrationTests {
	@LocalServerPort
	private int port;
	...
	public void setUp() throws Exception {
		baseUrl = "http://localhost:".concat("" + port).concat("/settings/configuration");
		buildToken();
	}
	...
	public void saveParameterTest() throws Exception {
		...
		Parameter parameter =Parameter.builder().code("prametreCodeTest").parameters(parameters).id(2L).build();
		...
		ResponseEntity<?> response = restClient.post().uri(baseUrl + "/save")
		    ...
		    .body(parameter).retrieve().toEntity(Parameter.class);
	...
	@Order(2)
	@Commit
	public void buildAutoParameterTest() throws Exception {
		
		ListChangeHandler<Parameter> parameters = new ListChangeHandler<>();
		parameters.addNew(Parameter.builder().code("prametre").parameters(null).id(5L).build());
		Parameter parameter=Parameter.builder().code("billing.role").parameters(parameters).id(2L).build();
		...
		ResponseEntity<?> response = restClient.post().uri(baseUrl + "/build-automatically")
		    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
		    .body(EditorData.class).retrieve().toEntity(EditorData.class);
