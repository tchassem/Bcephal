...
public class AutoRecoCumulatedRunner extends AutoRecoMethodRunner{
		...
		try{
	        ...
	        while(!rows.isEmpty() && count > 0 && !stop) { 
	        	...
	        	for(Object[] row : rows) { 
	        		...
	        		log.trace("[{} - {} - {}]", (count+1), primaryRowId, primaryAmount);
					...
					//List<Long> secondaryRowOids = new ArrayList<>(0);	        				        		
...
//		            			listener.createSubInfo(null, "Line" + count);		            			
	            		...
	            		if(this.getReco().isUseCombinations()) {
	            			card = getReco().isBuildCombinationsAsc() ? 1 : cardinality;
	            		}	            		
	            		while((cardinality >= card && card != 0) && !reconciliated && !stop) {
	            			if(stop) { return;}	
	            			log.trace("Cardinality = {}", card);
		            		Set<Set<Integer>> combinations = util.combinaison(elts, card);
		            		if(this.getReco().isUseCombinations()) {
		            			card = getReco().isBuildCombinationsAsc() ? card + 1 : card - 1;
		            		}	
		            		else {
		            			card = 0;
		            		}
		            		...
		            		while(iterator.hasNext() && !stop) {
		            			...
		            			List<Long> setOids = new ArrayList<>(0);
		            			for(Integer i : set) {
		            				if(stop) { return;}			
		            				if(isMaxDurationReash(maxDuration, startDate)) {
		            					cardinality = 0;
		            				...
		            				setOids.add(((Number)r[0]).longValue());
		            			...
		            			secondaryRowIds = new ArrayList<>(setOids);
		            			boolean valid = validateAmount(primaryAmount, setAmount, amounts, data);
			                	if(valid) {
			                		if(stop) { return;}			                		
			                		log.trace("{} - {} => {}", primaryRowId, setOids, setAmount);
			                		if(getReco().isLeftGridPrimary()) {
			                	...
			                	if(listener != null && listener.getInfo().getCurrentSubTask() != null 
			            			...
			            			if(listener.getInfo().getCurrentSubTask().getCurrentSubTask().getCurrentStep() % 15000 == 0) {
			            				listener.OnChange();
			            			}
				                }
			...
			throw new BcephalException(message, e);
		}
		finally {
			dropViews();
		}
