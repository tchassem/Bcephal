...
public class AutoRecoUnionViewQueryBuilder  extends AutoRecoUnionQueryBuilder {
	
	String viewName;
	
	String measureCol = "measure";
	String measure2Col = "measure2";
	
	/**
	 * @param userSession
	 */
	public AutoRecoUnionViewQueryBuilder(AutoReco reco, boolean forSecondaryGrid, EntityManager entityManager, String viewName) {
		super(reco, forSecondaryGrid, entityManager);
		this.viewName = viewName;
	}
	
	public AutoRecoUnionViewQueryBuilder(AutoReco reco, boolean forSecondaryGrid, EntityManager entityManager, String viewName, ReconciliationModelSide side, boolean forPartial) {
	...
	protected String buildSelectPart() {
		...
		if(measureCol != null) {
			selectPart += ", " + measureCol;
//			this.measures.put(measure.getId(), measure);
		}	
		if(measure2Col != null) {
			selectPart += ", " + measure2Col;
	...
	private String buildCommonAttributeSqlWherePart() {
		...
		for(ReconciliationCondition item : reco.getSortedConditions()) {
			...
			if(StringUtils.hasText(s)) {
				...
				if(org.springframework.util.StringUtils.hasText(verb)) {
					sql += " " + verb + " " ;
				}				
				sql += s;
			}
...
//						sql = col + " = " + amount1;
	...
	protected String buildOrderPart() {
		String sql = super.buildOrderPart();
//		String coma = "";
