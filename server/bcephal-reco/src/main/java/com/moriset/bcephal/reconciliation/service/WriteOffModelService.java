...
import com.moriset.bcephal.reconciliation.domain.WriteOffModel;
import com.moriset.bcephal.reconciliation.repository.WriteOffModelRepository;
...
@Service
@Slf4j
public class WriteOffModelService extends PersistentService<WriteOffModel, BrowserData> {


	@Autowired
	WriteOffModelRepository writeOffModelRepository;
	
	@Autowired
	WriteOffFieldService writeOffFieldService;
	
	@Override
	public WriteOffModelRepository getRepository() {
		return writeOffModelRepository;
	}

	
	@Transactional
	public WriteOffModel save(WriteOffModel writeOffModel, Locale locale) {
		...
		try {	
			...
			ListChangeHandler<WriteOffField> field = writeOffModel.getFieldListChangeHandler();
			
			writeOffModel = getRepository().save(writeOffModel);
			WriteOffModel id = writeOffModel;
	...
	@Override
	public void delete(WriteOffModel writeOffModel) {
