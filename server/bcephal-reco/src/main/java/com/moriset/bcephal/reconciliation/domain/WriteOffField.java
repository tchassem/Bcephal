 ...
 *
 */
@Entity(name = "WriteOffField")
@Table(name = "BCP_WRITE_OFF_FIELD")
@Data
@EqualsAndHashCode(callSuper = false)
public class WriteOffField extends Persistent {

	private static final long serialVersionUID = 5601887429814900356L;

	@Id
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "writeoff_field_seq")
	@SequenceGenerator(name = "writeoff_field_seq", sequenceName = "writeoff_field_seq", initialValue = 1,  allocationSize = 1)
	private Long id;
	...
	private List<WriteOffFieldValue> values;

	@Transient
	private ListChangeHandler<WriteOffFieldValue> valueListChangeHandler;
	...
	public WriteOffField() {
		...
		valueListChangeHandler = new ListChangeHandler<WriteOffFieldValue>();
		dateValue = new PeriodValue();
	}
	
	public void setFields(List<WriteOffFieldValue> values) {
		this.values = values;
	...
	@JsonIgnore
	public List<WriteOffFieldValue> getSortedValues() {
		List<WriteOffFieldValue> conditions = getValueListChangeHandler().getItems();
		Collections.sort(conditions, new Comparator<WriteOffFieldValue>() {
			@Override
			public int compare(WriteOffFieldValue item1, WriteOffFieldValue item2) {
	...
	@JsonIgnore
	@Override
	public WriteOffField copy() {
		WriteOffField copy = new WriteOffField();
		...
		for(WriteOffFieldValue value : getValueListChangeHandler().getItems()) {
