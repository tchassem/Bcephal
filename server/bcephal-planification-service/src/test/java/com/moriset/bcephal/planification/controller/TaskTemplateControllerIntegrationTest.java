...
import com.moriset.planification.TaskTemplateFactory;
...
public class TaskTemplateControllerIntegrationTest {
	...
	public void setUp() throws Exception {
		baseUrl = "http://localhost:".concat("" + port).concat("/planification/task-template");
		buildToken();
	}
	
	@Test
	@DisplayName("Save task-Template")
	@Order(1)
	@Commit
	public void saveTaskTest() throws Exception {
		interceptor.setTenantForServiceTest("" + BCephalParameterTest.PROJECT_CODE);
		Task task = TaskTemplateFactory.BuildtaskTemplate();
		
		assertThat(task).isNotNull();
		assertThat(task.getId()).isNull();
		
		ResponseEntity<Task> response = restClient.post().uri(baseUrl + "/save")
			    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
			    .body(task).retrieve().toEntity(Task.class);
			
			Assertions.assertThat(response).isNotNull();
			Assertions.assertThat(response.getStatusCode()).isNotNull();
			Assertions.assertThat(response.getStatusCode().value()).isEqualTo(200);
			Task newMeasure = response.getBody();
...
//		interceptor.setTenantForServiceTest("" + BCephalParameterTest.PROJECT_CODE);
//		
//			Task item = taskService.getByName("EOM voorbereiding");
//			long id = item.getId();
//			List<Long> ids = new ArrayList<>();
//			ids.add(id);
//					ResponseEntity<Boolean> responseToDelete = restClient.post().uri(baseUrl + "/delete-items")
//						    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
//						    .body(ids).retrieve().toEntity(Boolean.class);
//			
//			Assertions.assertThat(responseToDelete.getStatusCode().value()).isEqualTo(200);
//			
//	}
