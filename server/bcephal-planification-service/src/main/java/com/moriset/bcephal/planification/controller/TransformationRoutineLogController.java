...
import com.moriset.bcephal.planification.service.TransformationRoutineLogService;
...
@RestController
@RequestMapping("/planification/routine-log")
@Slf4j
public class TransformationRoutineLogController {
	
	@Autowired
	TransformationRoutineLogService logService;
	...
	public ResponseEntity<?> search(@RequestBody BrowserDataFilter filter,
		...
		try {
			BrowserDataPage<TransformationRoutineLog> page = logService.search(filter, locale);
