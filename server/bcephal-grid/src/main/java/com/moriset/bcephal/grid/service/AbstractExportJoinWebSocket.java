...
public class AbstractExportJoinWebSocket extends CustomAbstractWebSocketHandler {
	...
	protected MainObjectService<? extends MainObject, JoinBrowserData> getService() {
		return joinService;
	}
	...
	protected String getFunctionalityCode() {
		...
		return joinService.getFunctionalityCode();
	}
	...
	public void handleTextMessage(WebSocketSession session, TextMessage message)
		...
		log.info("Join export session id " + session.getId());
		refresh(session);
		...
		if ("STOP".equals(message.getPayload())) {
			...
			if (data != null && data.getFilter().getJoin() != null) {
				String tenantId = TenantContext.getCurrentTenant();
				data.getFilter().setJoin(joinService.getById(data.getFilter().getJoin().getId()));
						...
						try {
							...
							if (success) {
								...
								if (GrilleType.DASHBOARD_REPORT.equals(type)) {
									...
									joinService.saveUserSessionLog(session.getPrincipal().getName(),
	...
	private void export(java.util.Locale locale) {
		try {
			...
			if (data.getFilter().getJoin() != null) {
				buildGrid();
				dataId = data.getFilter().getJoin().getId();
				List<String> paths = joinService.export(data, null);
				if (listener != null) {
					listener.createInfo(data.getFilter().getUserId(), data.getFilter().getJoin().getName());
			...
			log.error("unexpected error while running Join : {}",
					data.getFilter() != null && data.getFilter().getJoin() != null
							? data.getFilter().getJoin().getName()
							: "",
					e);
			if (listener != null) {
	...
	private JoinExportData deserialize(String json) {
		try {
			return mapper.readValue(json, JoinExportData.class);
	...
	public void sendExportDataTransfert(ExportDataTransfert dataTranfert) {
		...
		try {
			...
			log.error("Fail to serialize data", data.getFilter().getJoin().getName());
			listener.error("Fail to serialize  data" + data.getFilter().getJoin().getName(), false);
		}
	}
}
