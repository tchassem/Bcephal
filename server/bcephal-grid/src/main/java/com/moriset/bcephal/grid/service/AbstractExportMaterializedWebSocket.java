...
public class AbstractExportMaterializedWebSocket extends CustomAbstractWebSocketHandler {
	...
	protected String getFunctionalityCode() {
		...
		return materializedGridService.getFunctionalityCode();
	}
	...
	public void handleTextMessage(WebSocketSession session, TextMessage message)
		...
		if ("STOP".equals(message.getPayload())) {
			...
			if (data != null && data.getFilter().getGrid() != null) {
				...
				data.getFilter().setGrid(materializedGridService.getById(data.getFilter().getGrid().getId()));
						...
						try {
							...
							if (success) {
								...
								if (GrilleType.DASHBOARD_REPORT.equals(type)) {
									...
									materializedGridService.saveUserSessionLog(session.getPrincipal().getName(),
	...
	private void export(java.util.Locale locale) {
		try {
			...
			if (data.getFilter().getGrid() != null) {
				...
				List<String> paths = materializedGridService.export(data, null);
				if (listener != null) {
			...
			log.error("unexpected error while running file loader : {}",
					data.getFilter() != null && data.getFilter().getGrid() != null
							? data.getFilter().getGrid().getName()
							: "",
					e);
			if (listener != null) {
	...
	private MaterializedExportData deserialize(String json) {
		try {
			return mapper.readValue(json, MaterializedExportData.class);
