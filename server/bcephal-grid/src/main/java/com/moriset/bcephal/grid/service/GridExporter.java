...
public class GridExporter {
	...
	public GridExporter(Grille grid, List<Long> ids, boolean exportAllColumns) {
		this();
	...
	public GridExporter(XSSFWorkbook workbook,Grille grid, List<Long> ids, boolean exportAllColumns) {
		this();
		boolean isOld = workbook != null;
		sworkbook = isOld ? new SXSSFWorkbook(workbook, -1) : new SXSSFWorkbook(-1);
		if(isOld) {
			if(sworkbook.getNumberOfSheets() > 0) {
			//	ssheet = sworkbook.getSheetAt(0);
			}
			if(sworkbook.getNumCellStyles() > 0) {
				style = sworkbook.getCellStyleAt(0);
			}
		}
		if(ssheet == null) {
			ssheet = sworkbook.createSheet("Data" + System.currentTimeMillis());
		}
		if(style == null) {
			style = sworkbook.createCellStyle();
		}
	...
	public void export(String path, Grille grid, List<Object[]> objects, boolean ignoreLastColumn) throws IOException {
	...
	public void export(XSSFWorkbook workbook,String path, Grille grid, List<Object[]> objects, boolean ignoreLastColumn) throws IOException {
		boolean isOld = workbook != null;
		this.workbook = workbook == null ? new XSSFWorkbook() : workbook;
		if(isOld) {
	...
	protected void writeHeaders(Grille grid) {
	...
	protected void writeHeaderss(Grille grid) {
	...
	public void writeFile(OutputStream fileOut) throws IOException {
	...
	public void writeFiles(OutputStream fileOut) throws IOException {
