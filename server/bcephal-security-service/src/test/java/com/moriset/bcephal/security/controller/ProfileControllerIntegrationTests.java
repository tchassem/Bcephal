...
import com.moriset.bcephal.security.ProfileFactory;
...
public class ProfileControllerIntegrationTests {
	...
	public void setUp() throws Exception {
		baseUrl = "http://localhost:".concat("" + port).concat("/profiles");
		buildToken();
	}
	...
	public void saveProfileTest() throws Exception {
		...
		List<Profile> profiles = ProfileFactory.BuildProfiles();
		...
		ResponseEntity<Profile> response = restClient.post().uri(baseUrl + "/save")
		    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
		    .body(profile).retrieve().toEntity(Profile.class);
		
		Assertions.assertThat(response).isNotNull();
		Assertions.assertThat(response.getStatusCode()).isNotNull();
		Assertions.assertThat(response.getStatusCode().value()).isEqualTo(200);
		Profile profile_= response.getBody();
