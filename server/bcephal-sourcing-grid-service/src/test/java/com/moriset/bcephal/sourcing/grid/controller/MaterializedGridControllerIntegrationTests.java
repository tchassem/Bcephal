...
public class MaterializedGridControllerIntegrationTests {
	...
	private static List<MaterializedGrid> grids;
	
	@Autowired MaterializedGridController materializedGridController;
	@Autowired MaterializedGridService gridService;
...
//		@Test
//		@DisplayName("Save all materialize grids")
//		@Order(1)
//		@Commit
		public void saveMaterializeGridTest() throws Exception {
			Interceptor.setTenantForServiceTest("" + BCephalParameterTest.PROJECT_CODE);
			grids = MaterializedGilleFactory.buildMaterializedGrilles();
			Assertions.assertThat(grids).isNotNull();
			
			for(var grid : grids) {
				ResponseEntity<?> response = restClient.post().uri(baseUrl + "/save")
...
//		@Test
//		@DisplayName("Create Report Grid")
//		@Order(2)
//		@Commit
//		public void createReportTest() throws Exception {
//			Interceptor.setTenantForServiceTest("" + BCephalParameterTest.PROJECT_CODE);
//			MaterializedGrid grid = MaterializedGilleFactory
//									.buildBIL310SchemeFeeDedicatedFeeNotAllocatedToABillableMember();
//			Assertions.assertThat(grid).isNotNull();
//			MaterializedGrid gridId = gridService.save(grid, Locale.getDefault());
//			Assertions.assertThat(grid.getId()).isNotNull();
//			ResponseEntity<?> response = restClient.post().uri(baseUrl + "/create-report/"+ gridId.getId())
//			    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
//			    .body("ReportTest1").retrieve().toEntity(MaterializedGrid.class);
//			
//			Assertions.assertThat(response).isNotNull();
//			Assertions.assertThat(response.getStatusCode()).isNotNull();
//			Assertions.assertThat(response.getStatusCode().value()).isEqualTo(200);
