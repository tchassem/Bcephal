...
public class MaterializeControllerIntegrationtests {
	...
	@Autowired
	MaterializedGridService grilleService;
	...
	private static MaterializedGridDataFilter filter;
	
	private static String reportName = "Report grille";
	
	@BeforeAll
    static void init() {
		log.info("Begin of Before all tests");
    	restClient = RestClient.builder().messageConverters(addConverters()).build();
    	grille = CreateFactory.buildMaterialize();
    	filter = new MaterializedGridDataFilter();
	...
	public void setUp() throws Exception {
		baseUrl = "http://localhost:".concat("" + port).concat("/sourcing/mat-grid");
		buildToken();
	}
	
	@Test
	@DisplayName("Save matGrid")
	@Order(1)
	@Commit
	public void saveGrilleTest() throws Exception {
		...
		ResponseEntity<MaterializedGrid> response = restClient.post().uri(baseUrl + "/save")
		    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
		    .body(grille).retrieve().toEntity(MaterializedGrid.class);
	...
	@Test
	@DisplayName("Set editable matGrid")
	@Order(3)
	@Commit
	public void setEditableGrilleTest() throws Exception {
		...
		assertThat(response).isNotNull();
		MaterializedGrid grid = grilleService.getById(grille.getId());
		assertThat(grid).isNotNull();
		assertThat(grid.isEditable()).isTrue();
	}
	
	@Test
	@DisplayName("SearchRows of matGrid")
	@Order(4)
	@Commit
	...
	@Test
	@DisplayName("SearchRows2 of matGrid")
	@Order(5)
	public void searchRows2Test() throws Exception {
		...
		ResponseEntity<BrowserDataPage<GridItem>> response = restClient.post().uri(baseUrl + "/rows2")
		    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
		    .body(filter).retrieve().toEntity(new ParameterizedTypeReference<BrowserDataPage<GridItem>>() {});
	...
	public void publishGrilleTest() throws Exception {
		...
		ResponseEntity<EditorData<MaterializedGrid>> response = restClient.get().uri(baseUrl + "/publish/" + grille.getId())
		    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
		    .retrieve().toEntity(new ParameterizedTypeReference<EditorData<MaterializedGrid>>() {});
		
		assertThat(response).isNotNull();		
		EditorData<MaterializedGrid> editorData = response.getBody();
		assertThat(editorData).isNotNull();
		assertThat(editorData.getItem()).isNotNull();
		assertThat(editorData.getItem().isPublished()).isTrue();
	}
	
	@Test
	@DisplayName("publish matGrids")
	@Order(7)
	@Commit
	...
	@Test
	@DisplayName("Reset publication matGrid")
	@Order(8)
	@Commit
	...
	@Test
	@DisplayName("Reset publication matGrids")
	@Order(9)
	@Commit
	...
	@Test
	@DisplayName("Refresh publication matGrid")
	@Order(10)
	@Commit
	...
	@Test
	@DisplayName("Refresh publication matGrids")
	@Order(11)
	@Commit
	...
	@Test
	@DisplayName("Get columns matGrid")
	@Order(12)
	@Commit
	...
	public void deleteReportGrilleTest() throws Exception {
		...
		ResponseEntity<MaterializedGrid> response = restClient.get().uri(baseUrl + "/by-name/" + reportName)
		    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
		    .retrieve().toEntity(MaterializedGrid.class);
		
		assertThat(response).isNotNull();
		assertThat(response.getStatusCode()).isNotNull();
		assertThat(response.getStatusCode().value()).isEqualTo(200);
		
		MaterializedGrid grid  = response.getBody();
		assertThat(grid).isNotNull();
		...
		assertThat(grille.getId()).isNotNull();
		MaterializedGrid grid_ = grilleService.getById(grille.getId());
