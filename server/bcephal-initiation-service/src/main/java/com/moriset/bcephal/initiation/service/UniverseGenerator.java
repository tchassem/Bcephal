...
public class UniverseGenerator {
	...
	public void createUniverseColumn(Dimension dimension) throws Exception {
		if (dimension.getId() != null) {
			String columnName = dimension.getUniverseTableColumnName();
			String type = dimension.getUniverseTableColumnType();
			if (!StringUtils.isBlank(columnName)) {
				createUniverseColumn(columnName, type);
			}
		}
	}

	public void dropUniverseColumn(Dimension dimension) {
		if (dimension.getId() != null) {
			String columnName = dimension.getUniverseTableColumnName();
			if (!StringUtils.isBlank(columnName)) {
				boolean isEmpty = isUniverseColumnEmpty(columnName, false);
				if (isEmpty) {
					dropUniverseColumn(columnName);
				}
			}
		}
	}
