...
public class PeriodControllerIntegrationTests {
	...
	public void setUp() throws Exception {
		baseUrl = "http://localhost:".concat("" + port).concat("/initiation/period");
		buildToken();
	}

	@Test
	@DisplayName("Save periods")
	@Order(1)
	@Commit
	public void savePeriodsTest() throws Exception {
		
		PeriodName periodName = new InitiationFactory().buildMainPeriod();
		...
		ResponseEntity<PeriodName> response = restClient.post().uri(baseUrl + "/save")
		    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
		    .body(periodName).retrieve().toEntity(PeriodName.class);
		
		Assertions.assertThat(response).isNotNull();
		Assertions.assertThat(response.getStatusCode()).isNotNull();
		Assertions.assertThat(response.getStatusCode().value()).isEqualTo(200);
		PeriodName newPeriod = response.getBody();
		Assertions.assertThat(newPeriod).isNotNull();
