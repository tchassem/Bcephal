...
import com.moriset.bcephal.initiation.config.HashMapHttpMessageConverter;
...
@Slf4j
public class MeasureControllerIntegrationTests {
	...
	public void setUp() throws Exception {
		baseUrl = "http://localhost:".concat("" + port).concat("/initiation/measure");
		buildToken();
	}

	@Test
	@DisplayName("Save Measure")
	@Order(1)
	@Commit
	public void saveMeasuresTest() throws Exception {
		
		Measure measure = new InitiationFactory().buildMeasure();
		...
		ResponseEntity<Measure> response = restClient.post().uri(baseUrl + "/save")
		    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
		    .body(measure).retrieve().toEntity(Measure.class);
		
		Assertions.assertThat(response).isNotNull();
		Assertions.assertThat(response.getStatusCode()).isNotNull();
		Assertions.assertThat(response.getStatusCode().value()).isEqualTo(200);
		Measure newMeasure = response.getBody();
		Assertions.assertThat(newMeasure).isNotNull();
