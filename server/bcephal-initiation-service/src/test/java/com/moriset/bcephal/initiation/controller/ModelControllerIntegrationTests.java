...
public class ModelControllerIntegrationTests {
	...
	public void setUp() throws Exception {
		baseUrl = "http://localhost:".concat("" + port).concat("/initiation/model");
		buildToken();
	}

	@Test
	@DisplayName("Save all Model")
	@Order(1)
	@Commit
	public void saveAllModelsTest() throws Exception {
		
		ListChangeHandler<Model> models = new InitiationFactory().BuildModels();
		...
		ResponseEntity<?> response = restClient.post().uri(baseUrl + "/save-all")
		    .headers(httpHeaders -> httpHeaders.addAll(buildRequestHeaders()))
		    .body(models).retrieve().toEntity(Model.class);
