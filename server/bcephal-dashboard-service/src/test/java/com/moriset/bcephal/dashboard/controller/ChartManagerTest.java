...
import com.moriset.bcephal.dashboard.factory.ChartFactory;
...
public class ChartManagerTest {
	...
	private static List<DashboardReport> charts;
    ...
    @Test
	@DisplayName("Create all charts")
	@Order(1)
	@Commit
	public void saveChartTest() throws Throwable {
		interceptor.setTenantForServiceTest("" + BCephalParameterTest.PROJECT_CODE);
		charts = ChartFactory.buildCharts();
		assertThat(charts).isNotNull();
		for(DashboardReport report : charts) {
			...
			ResponseEntity<DashboardReport> response = restClient.post().uri(baseUrl + "/save")
				    ...
				    .body(report).retrieve().toEntity(DashboardReport.class);
