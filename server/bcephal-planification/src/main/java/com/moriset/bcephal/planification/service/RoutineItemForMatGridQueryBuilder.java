...
public class RoutineItemForMatGridQueryBuilder  extends MaterializedGridQueryBuilder {

	TransformationRoutineItem item;
	Map<String, Object> parameters;
	LoaderData loaderData;
	
	public RoutineItemForMatGridQueryBuilder(MaterializedGridDataFilter filter, TransformationRoutineItem item) {
		super(filter);
		this.item = item;
...
//					String targetTable = getFilter().getGrid().getMaterializationTableName();
...
//				}	
			...
			if(StringUtils.hasText(setpart)){
				sql = "UPDATE " + getFilter().getGrid().getMaterializationTableName() + " item";
				sql += setpart;
				String wherePart = buildWherePart(column);
				if(StringUtils.hasText(wherePart)){
					sql += wherePart;
	...
	protected String buildSetPart(String col) {		
		...
		if(item.getSourceField().getSourceType() == TransformationRoutineSourceType.FREE) {
			...
			sql = buildSetSpotPart(col);
		}
		return sql;
	}
	
	private String buildSetSpotPart(String col) {
		String sql = null;
	...
	private String buildLoadDataWherePart() {
		...
		String coma = "";
		if(loaderData != null) {
			if(StringUtils.hasText(loaderData.getLoaderName())) {
//				MaterializedGridColumn column = getFilter().getGrid().getColumnByCategory(GrilleColumnCategory.LOAD_);
//				if(column != null) {
//					sql += coma + column.getDbColumnName() + " = '" + loaderData.getLoaderName() + "'";
//					coma = " AND ";
//				}
			}
			if(StringUtils.hasText(loaderData.getLoadNbr())) {
				MaterializedGridColumn column = getFilter().getGrid().getColumnByCategory(GrilleColumnCategory.LOAD_NBR);
				if(column != null) {
					String param = "param_" + column.getId() + "_" + System.currentTimeMillis();
					parameters.put(param, loaderData.getLoadNbr());
					sql += coma + column.getDbColumnName() + " = :" + param;
					coma = " AND ";
					
				}
			}
			if(StringUtils.hasText(loaderData.getUsername())) {
				MaterializedGridColumn column = getFilter().getGrid().getColumnByCategory(GrilleColumnCategory.LOAD_USER);
				if(column != null) {
					String param = "param_" + column.getId() + "_" + System.currentTimeMillis();
			...
			if(loaderData.getMode() != null) {
				MaterializedGridColumn column = getFilter().getGrid().getColumnByCategory(GrilleColumnCategory.LOAD_MODE);
				if(column != null) {
					String param = "param_" + column.getId() + "_" + System.currentTimeMillis();
					parameters.put(param, loaderData.getMode().name());
					sql += coma + column.getDbColumnName() + " = :" + param;
					coma = " AND ";
				}
			}
			if(loaderData.getLoadDate() != null) {
				MaterializedGridColumn column = getFilter().getGrid().getColumnByCategory(GrilleColumnCategory.LOAD_DATE);
				if(column != null) {
					String param = "param_" + column.getId() + "_" + System.currentTimeMillis();
					parameters.put(param, loaderData.getLoadDate());
					sql += coma + column.getDbColumnName() + " = :" + param;
					coma = " AND ";
				}
			}
			if(loaderData.getFiles() != null && loaderData.getFiles().size() > 0) {
				MaterializedGridColumn column = getFilter().getGrid().getColumnByCategory(GrilleColumnCategory.LOAD_SOURCE_NAME);
				if(column != null) {
					String param = "param_" + column.getId() + "_" + System.currentTimeMillis();
					parameters.put(param, loaderData.getFiles());
					sql += coma + column.getDbColumnName() + " in :" + param;					
					coma = " AND ";
				}
			}
			if(StringUtils.hasText(loaderData.getOperationCode())) {
				MaterializedGridColumn column = getFilter().getGrid().getColumnByCategory(GrilleColumnCategory.OPERATION_CODE);
				if(column != null) {
					String param = "param_" + column.getId() + "_" + System.currentTimeMillis();
					parameters.put(param, loaderData.getOperationCode());
					sql += coma + column.getDbColumnName() + " = :" + param;
					coma = " AND ";
