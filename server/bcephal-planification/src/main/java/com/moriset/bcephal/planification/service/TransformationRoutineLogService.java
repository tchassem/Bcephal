...
@Service
@Slf4j
public class TransformationRoutineLogService extends PersistentService<TransformationRoutineLog, TransformationRoutineLog> {

	@Autowired
	TransformationRoutineLogRepository logRepository;
	
	@Autowired
	TransformationRoutineLogItemRepository logItemRepository;

	@Override
	public TransformationRoutineLogRepository getRepository() {
	...
	@Override
	public void delete(TransformationRoutineLog item) {
		log.debug("Try to delete log : {}", item.getId());
		if (item == null || item.getId() == null) {
			return;
		}
		getRepository().deleteById(item.getId());
		logItemRepository.deleteByLogId(item.getId());
		log.debug("Log successfully deleted : {}", item.getId());
	}

	public BrowserDataPage<TransformationRoutineLog> search(BrowserDataFilter filter, java.util.Locale locale) {
		if (filter.getPage() == 0) {
			filter.setPage(1);
		}
		BrowserDataPage<TransformationRoutineLog> page = new BrowserDataPage<TransformationRoutineLog>();
		page.setPageSize(filter.getPageSize());
		Specification<TransformationRoutineLog> specification = getBrowserDatasSpecification(filter, locale);

		if (filter.isShowAll()) {
			List<TransformationRoutineLog> items = getRepository().findAll(specification, Sort.by(Order.desc("id")));
			page.setItems(items);
			...
			Page<TransformationRoutineLog> oPage = getRepository().findAll(specification,
	...
	protected Specification<TransformationRoutineLog> getBrowserDatasSpecification(BrowserDataFilter filter, Locale locale) {
		return (root, query, cb) -> {
			RequestQueryBuilder<TransformationRoutineLog> qBuilder = new RequestQueryBuilder<TransformationRoutineLog>(root, query, cb);
			qBuilder.select(TransformationRoutineLog.class);
